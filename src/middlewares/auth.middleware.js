...
module.exports = (passport) => {
    const options = {
        jwtFromRequest : ExtractJwt.fromAuthHeaderWithScheme('jwt'),//? busca en el header en  autorizacion lo q comuenza por jwt
        secretOrKey: jwtSecret /// viene de confi
    }
    passport.use(
        new JwtStrategy(options, async(decoded, done) => {
            //?decode el token decodificado
            //? done(error, decoded)
            try {
                const response = await getUserById(decoded.id)
                if(!response){
                    return done(null, false)//? no le pasaos error ni usurio pq no esta autorizado
                }
                console.log('decoded JWT', decoded)
                return done(null, decoded)//? aqui el usuario si decodificado
            } catch (error) {
